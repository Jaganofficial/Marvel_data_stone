...
@Composable
fun ShowAllTradePaperBack(showAllTradePaperBackViewModel: ShowAllTradePaperBackViewModel, sharedViewModel: SharedViewModel, navController: NavController) {
    ...
    Surface(modifier = Modifier.fillMaxSize()) {
        Column(modifier = Modifier.fillMaxSize(), horizontalAlignment = Alignment.CenterHorizontally) {
            ...
            var list= emptyList<com.example.marveldatastone.model.CharacterModels.TradePaperBackModel.Result>()

            if(showAllTradePaperBackViewModel.tradePaperBackListVM.collectAsState().value.isNotEmpty())
            {
                list=showAllTradePaperBackViewModel.tradePaperBackListVM.collectAsState().value[0].data.results
            }
            Column() {
                LazyColumn()
                {
