...
@Composable
fun ShowAllDigest(showAllDigestViewModel: ShowAllDigestViewModel, sharedViewModel: SharedViewModel, navController: NavController) {
    ...
    Surface(modifier = Modifier.fillMaxSize()) {
        Column(modifier = Modifier.fillMaxSize(), horizontalAlignment = Alignment.CenterHorizontally) {
            ...
            {
                list=showAllDigestViewModel.digestListVM.collectAsState().value[0].data.results
            }
            Column() {
                LazyColumn()
                {
